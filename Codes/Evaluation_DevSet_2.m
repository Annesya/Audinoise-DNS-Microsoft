...
figure;
subplot(3,1,1);
waterfall(t_idx,f_idx,noisy_lps);colormap jet; colorbar; view(0,90);axis xy; axis tight;
subplot(3,1,2);
waterfall(t_idx,f_idx,noisyIN);colormap jet; colorbar; view(0,90);axis xy; axis tight;
subplot(3,1,3);
waterfall(t_idx,f_idx,enhancedIN);colormap jet; colorbar; view(0,90);axis xy; axis tight;
%% Time domain reconstruction
j = sqrt(-1);
clean_estm_2 = istft(10.^(enhancedIN/2).*exp(j*phase_all),f_new,'Window',hamming(winLen,'periodic'),'OverlapLength',overlap,'FFTLength',fftLen);
clean_estm_1 = istft(10.^(noisyIN/2).*exp(j*phase_all),f_new,'Window',hamming(winLen,'periodic'),'OverlapLength',overlap,'FFTLength',fftLen);
noisy_org = istft(10.^(noisy_lps/2).*exp(j*phase_all),f_new,'Window',hamming(winLen,'periodic'),'OverlapLength',overlap,'FFTLength',fftLen);
